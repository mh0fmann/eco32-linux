...
static void eco32sdc_request_tasklet(unsigned long data)
{
	...
	spin_lock_irqsave(&eco32sdc_dev.q_lock, flags);
	...
	__blk_end_request_all(eco32sdc_dev.current_request, BLK_STS_OK);

	/* mark as free and start queue */
	eco32sdc_dev.busy = 0;
	blk_start_queue(eco32sdc_dev.queue);
	spin_unlock_irqrestore(&eco32sdc_dev.q_lock, flags);
...
static void eco32sdc_request_fn(struct request_queue* q)
{
	 ...
	 */
	if (eco32sdc_dev.busy || blk_queue_stopped(q)) {
		blk_stop_queue(q);
		return;
	}
	 ...
	 * if there is no request there is nothing to do for us
	 */
	if ((eco32sdc_dev.current_request = blk_fetch_request(q)) == NULL) {
		return;
	}
	...
	switch (req_op(eco32sdc_dev.current_request)) {
		...
		default: {
			...
			__blk_end_request_all(eco32sdc_dev.current_request, BLK_STS_IOERR);
			return;
		}
	}
...
static int eco32sdc_probe(struct platform_device* dev)
{
	...
	/* init queue lock */
	spin_lock_init(&eco32sdc_dev.q_lock);

	/* init request queue */
	if ((eco32sdc_dev.queue = blk_init_queue(eco32sdc_request_fn, &eco32sdc_dev.q_lock)) == NULL) {
